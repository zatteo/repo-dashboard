...
export function VersionStatus({ latestStable, latestBeta }: VersionStatusProps) {
	return (
		<div className="flex items-center gap-4 mb-3">
			{latestStable && (
				<div className="flex items-center gap-2">
					...
					<span
						...
						aria-label={`Published ${formatDate(latestStable.published_at || latestStable.created_at)}`}
					>
			...
			{latestBeta && (
				<div className="flex items-center gap-2">
					...
					<span
						className={`text-sm ${
							isOlderThan(latestBeta.published_at || latestBeta.created_at, 1 / 4.3) // ~1 week
								? 'text-red-400'
								: 'text-gray-500'
						}`}
						aria-label={`Published ${formatDate(latestBeta.published_at || latestBeta.created_at)}`}
					>
