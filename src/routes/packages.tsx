		...
		...packages.map((repo) =>
			columnHelper.accessor(repo.repo_name, {
				...
				cell: (info) => {
					...
					const textColor = getVersionColorClass(value, trackedPackage?.targetVersion)

					return <div className={`${textColor} font-mono text-sm`}>{value}</div>
				},
			})
		),
	]
	...
	return (
		<PageLayout>
			...
			<div className="mt-6 flex items-center gap-2 text-sm text-gray-400">
				...
				<span>
					Tracking {TRACKED_PACKAGES.length} key packages across all repositories
				</span>
			</div>
		</PageLayout>
	)
